...
function DialogOverlay({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Overlay>) {
  return (
    <DialogPrimitive.Overlay
      ...
      className={cn(
        // frosted, not just dimmed: the studio blurs away behind any modal
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          // w-[calc(100%-2rem)] keeps a margin on narrow phones; max-h +
          // overflow lets tall content scroll instead of spilling off-screen
          "bg-background data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-1/2 left-1/2 z-50 grid max-h-[calc(100dvh-2rem)] w-[calc(100%-2rem)] max-w-sm -translate-x-1/2 -translate-y-1/2 gap-4 overflow-y-auto rounded-lg border p-6 shadow-lg duration-200",
          className
        )}
        {...props}
      >
